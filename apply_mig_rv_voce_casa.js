...
(async () => {
  ...
  // Verifiche con operatori jsonb (NON LIKE su testo: jsonb::text mette uno
  // spazio dopo i due punti, "nome": "IMEI", e i LIKE senza spazio fallirebbero).
  ...
  console.log("Voce Casa → campo IMEI attivo:", (await q(
    "select count(*) n from catalog_campi_regole where condizioni->'offerta' ? 'Voce Casa' and exists (select 1 from jsonb_array_elements(campi) c where c->>'nome'='IMEI' and (c->>'attivo')::boolean is true)"))?.n, "(attesa 1)");
  console.log("Outdoor → ICCID nascosto (attivo=false):", (await q(
    "select count(*) n from catalog_campi_regole where etichetta like '%Outdoor%no ICCID%' and exists (select 1 from jsonb_array_elements(campi) c where c->>'nome'='Seriale SIM (ICCID)' and (c->>'attivo')::boolean is false)"))?.n, "(attese 2)");
  console.log("regola generale FWA (16c3b3d6) ancora con ICCID attivo:", (await q(
    "select exists (select 1 from jsonb_array_elements(campi) c where c->>'nome'='Seriale SIM (ICCID)' and (c->>'attivo')::boolean is true) ok from catalog_campi_regole where id='16c3b3d6-7a97-402e-8102-d73422457d04'"))?.ok, "(atteso true — invariata)");
