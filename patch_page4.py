...
source_file = r"e:\PRTFLIO\LUCA CRM\replica\CRM_v9_2.jsx"
target_file = r"e:\PRTFLIO\LUCA CRM\replica\src\app\(dashboard)\registra-contratto\page.tsx"
...
"""
# Assuming the file doesn't have imports yet, we prepend it.
content = imports + "\n" + content

# 2. Add states for attachments and uploading
state_injection = """  const [toast,setToast]=useState(null);
  const showToast = (msg) => {
    setToast(msg);
...
"""
content = re.sub(r"const\s+\[toast,setToast\]\s*=\s*useState\(null\);", state_injection, content)
...
# 5. Handle Download PDF and finalSubmit
...
content = re.sub(old_finalSubmit_regex, lambda m: new_finalSubmit, content)
...
"""
content = re.sub(r"(<div style=\{\{display:\"flex\",gap:12,marginTop:24,flexWrap:\"wrap\",alignItems:\"center\"\}\}>)", lambda m: uploading_ui + m.group(1), content)
...
content = re.sub(r"(<button onClick=\{\(\)=>setShowCart\(false\)\}.*?</button>)", lambda m: m.group(1) + "\n" + pdf_btn, content)

# 9. Dynamic SSR wrapper at the end
import_dynamic = """
import dynamic from "next/dynamic";
const CRM_NoSSR = dynamic(() => Promise.resolve(CRM), { ssr: false });
export default CRM_NoSSR;
"""
content = re.sub(r"export default function CRM\(\) \{", "function CRM() {", content)
content = content + "\n" + import_dynamic
