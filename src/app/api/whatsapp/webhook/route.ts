...
// upsert conversazione (istanza + numero cliente), aggancio cliente per numero
async function upsertConversazione(instanceId: string, numero: string, nome: string | null) {
    ...
    const { data: existing } = await supabase.from("wa_conversations")
        .select("id, client_id, customer_name").eq("instance_id", instanceId).eq("customer_number", numero).maybeSingle();
    if (existing) {
    ...
    const { data: created } = await supabase.from("wa_conversations")
        .insert({ instance_id: instanceId, customer_number: numero, customer_name: nome, client_id: clientId })
        .select("id").single();
    ...
    try {
        ...
        if (event.includes("messages.upsert") && inst && data) {
            ...
            for (const m of messages) {
                const key = m?.key || {};
                if (key.remoteJid?.endsWith("@g.us")) continue;      // ignora i gruppi
                const fromMe = !!key.fromMe;
                const numero = String(key.remoteJid || "").split("@")[0];
                if (!numero) continue;
                const convId = await upsertConversazione(inst.id, numero, m?.pushName || null);
                const msg = m?.message || {};
