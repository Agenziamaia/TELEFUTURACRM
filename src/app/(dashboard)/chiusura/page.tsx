            ...
            <div className="sticky top-0 z-30 bg-[#0f111a]/85 backdrop-blur-sm border-b border-white/5">
                <div className="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 py-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {/* IL REPORT DELLA SERA (Luca 28/08): a sinistra di
                            «Invio e Chiusura», perché è lo stesso momento della
                            giornata. Sta FUORI dal modulo dei documenti: il
                            report si manda anche senza allegare niente. */}
                        <button onClick={() => setReport(true)} disabled={!storeReport}
                            title={storeReport ? `Report della giornata — ${storeReport}` : "Nessun negozio associato al tuo account"}
                            className={cn("flex items-center gap-1.5 px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl text-sm font-semibold transition-all",
                                storeReport ? "bg-indigo-500/15 text-indigo-300 border border-indigo-500/40 hover:bg-indigo-500/25"
                                    : "bg-white/5 text-slate-600 border border-white/5 cursor-not-allowed")}>
