...
import { NAVIGATION, effectiveAllowed, canSeeDefault, OUTBOUND_HIDDEN_GROUPS, type PermMap } from "@/lib/nav";
import { notify, dbError } from "./toast";

interface Riga { href: string; nome: string; gruppo?: string; defaultRoles: string[] }
...
function catalogo(): { titolo: string; voci: Riga[] }[] {
    ...
    NAVIGATION.forEach((e) => {
        ...
        else if (e.type === "hub") sciolte.push({ href: e.href, nome: `${e.name} (hub)`, defaultRoles: e.roles });
        else {
                        ...
                        <div key={g.titolo} className="rounded-xl bg-white/[0.02] border border-white/5 overflow-hidden">
                            ...
                            <div className="divide-y divide-white/5">
                                {g.voci.map((v) => {
                                    ...
                                    return (
                                        <div key={v.href} className="flex items-center gap-3 px-4 py-2.5">
