...
const SCd = ({session,codici,val,onCh}) => {
  ...
  const content = (
    <div>
      ...
      {/* la tendina del SISTEMA non si può vestire: il menu che si apre è
          quello del sistema operativo, blu e fuori posto (Luca 28/08). Qui
...
// CONVERGENZA SIM BUSINESS ↔ FISSO (Luca 25/08 sera, gara Fastweb): la sim
// business venduta insieme a un fisso vale +50 € di gara. Il dato non si
// deduceva da nulla: quando nello stesso carrello c'e' una sim business e si
// registra il fisso, si chiede conferma (come il pop-up del telefono a rate)
// e la conferma spunta l'opzione di catalogo SULLA SIM.
const OPZ_CONVERGENZA="Wireline contestuale";
const simConvergenzaAgganciabili=(cats,sales)=>{
  const out=[];
  (cats||[]).forEach(g=>((sales||{})[g.id]||[]).forEach((row,ri)=>(g.subs||[]).forEach(s2=>{
    const d=row&&row[s2.id];
    if(!d||!d.active||!s2.isCatalogo)return;
    if(s2.catTipo!=="Business"||!MOBILE_CATS_AGGANCIO.includes(s2.catCategoria))return;
    const f=d.fields||{};
    const off=f["Offerta"]||"";
    const oo=(s2.catOfferte||[]).find(o=>o.nome===off);
    // l'opzione deve esistere a CATALOGO per quell'offerta (data-driven: se
    // domani sparisce, sparisce anche il pop-up)
    if(!oo||!(oo.opzioni||[]).some(k=>k.nome===OPZ_CONVERGENZA))return;
    out.push({gid:g.id,si:ri,subId:s2.id,
      etichetta:off+" #"+(ri+1)+(_numeroMobile(f)?" · "+_numeroMobile(f):""),
      gia:!!(f.__opzioni||{})[OPZ_CONVERGENZA]});
  })));
  return out;
};
...
function CRM() {
  ...
  const _numeroRicarica=(idx,val)=>setMargItems(p=>p.map((m,i)=>
    i===idx&&m.paystore?{...m,paystore:{...m.paystore,numero:String(val||"").replace(/\D/g,"").slice(0,11),
      /* ⚠️ SCRITTO A MANO, e va detto: il ricalcolo delle voci automatiche
         scatta a ogni prodotto aggiunto e riprende il numero dalla vendita.
         Su Vodafone Business quel numero è VUOTO — il catalogo spegne i campi
         — quindi senza questo marchio il numero appena digitato spariva al
         primo tocco, ed è proprio il caso per cui il campo esiste. */
      aMano:true},
  ...
  // sim business del carrello che possono essere marcate «in convergenza»
  ...
  const computeAutoMarg=(prev,brandId,brandLabel,items)=>{
    ...
    const merged=adds.map(a=>{
      ...
      if(a.paystore&&old.paystore){
        ...
        const mg=tot*pct/100;
        return conNum({...a,paystore:{...a.paystore,pezzi:pz,importo:tot},
          importo:tot>0?tot:null,price:tot,margin:mg,totalMargin:mg});
      }
...
const _descrizioneConImei = (modello, seriale, fallback) => rigaConImei(modello, seriale, fallback);
  ...
  const cosaManca = () => {
    ...
    /* e il numero della ricarica: senza questa riga il pulsante restava verde
       e il blocco arrivava solo premendo Salva, senza dire dove rimediare —
       ed è il caso NUOVO, la ricarica associata alla SIM */
    margNumeroMancante(bObj ? computeAutoMarg(margItems, brand, bObj.label, colItems()) : margItems)
      .forEach(m => out.push({ ico: "📲", testo: `manca il numero da ricaricare per «${m.product}»`, dove: "carrello" }));
    if (posScontrinoAbilitato(selNeg)) senzaReparto(margItems).forEach(m => out.push({ ico: "🧾", testo: `«${m.product}» non ha un reparto IVA: lo scontrino non lo può stampare (Amministrazione → Fiscalità → Articoli)`, dove: "carrello" }));
