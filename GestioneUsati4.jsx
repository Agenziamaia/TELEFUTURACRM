...
function DevicePanel({ device, onClose, onSave }) {
  const [dev, setDev] = useState(() => ({ ...device, ricambi: device.ricambi.map(r => ({ ...r })), extra_margine: device.extra_margine ? { ...device.extra_margine } : null }));
  ...
  const handleSave = () => { onSave({ ...dev, note_tecnico: noteTecnico, sale_price: editSalePrice ? (parseFloat(salePriceVal) || 0) : 0 }); };
          ...
          <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 28 }}>
            ...
            <div>
              <div style={{ marginBottom: 20 }}>
                ...
                {dev.provenienza_subito && <div style={{ marginBottom: 10, display: "inline-flex", alignItems: "center", gap: 6, padding: "4px 12px", borderRadius: 20, fontSize: 11, fontWeight: 600, background: C.orange + "18", color: C.orange, border: "1px solid " + C.orange + "40" }}>{"\u{1F310}"} Provenienza Subito.it</div>}
...
function RegistraUsatoPanel({ onClose, onSave }) {
  ...
  const canNext = () => {
    ...
    if (step === 3) return brand && model && capacita && colore && imei && prezzoAcquisto && gradoUsura && (!hasExtraMargine || extraMargineImporto);
...
export default function GestioneUsati() {
  ...
  const resetFilters = () => { setSelectedStores([...NEGOZI]); setSelectedStatuses([...STATUS_KEYS]); setDateField("created_at"); setDateFrom(""); setDateTo(""); setSearchText(""); setRicambiFilter([]); };
